...
export default function PunchButtons({
  ...
  const handlePunchIn = async () => {
    ...
    try {
      ...
      stopScreenCapture();
      setScreenCaptureActive(false);
      setError(err.message || 'An error occurred. Please enable screen sharing.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white/40 backdrop-blur-xl rounded-2xl shadow-xl border border-white/50 p-6 h-full flex flex-col">
        ...
        <div className="mb-6">
          {status === 'on_leave' ? (
            ...
            <>
              <div className="flex items-center gap-3 bg-white/60 backdrop-blur-lg px-4 py-3 rounded-xl shadow-md border border-white/40">
                <div
                  className={`w-3 h-3 rounded-full ${status === 'not_punched_in'
                    ? 'bg-gray-400'
                    : status === 'punched_in'
                ...
                />
                <span className={`text-lg font-bold ${status === 'not_punched_in' ? 'text-gray-700' :
          ...
          {todayStatus?.record && status !== 'on_leave' && (
            <div className={`mt-4 p-5 rounded-2xl border-2 ${status === 'on_lunch_break'
              ...
              <div className="flex items-center justify-center gap-2">
                {/* Hours */}
                <div className={`flex flex-col items-center px-4 py-3 rounded-xl ${status === 'on_lunch_break'
                  ? 'bg-yellow-100'
                  : status === 'punched_out'
                    ? 'bg-blue-100'
                    : 'bg-emerald-100'
                  }`}>
                  <span className={`text-4xl font-bold font-mono ${status === 'on_lunch_break'
                    ? 'text-yellow-700'
                    : status === 'punched_out'
                      ? 'text-blue-700'
                      : 'text-emerald-700'
                ...
                <span className={`text-3xl font-bold ${status === 'on_lunch_break'
                  ? 'text-yellow-400'
                  : status === 'punched_out'
                    ? 'text-blue-400'
          ...
          {(status === 'punched_in' || status === 'on_lunch_break') && (
            <div className={`mt-3 flex items-center gap-2 text-sm ${status === 'on_lunch_break' || screenCapturePaused
              ? 'text-yellow-600'
              : screenCaptureActive
                ? 'text-green-600'
                : 'text-red-600'
              }`}>
              <div className={`w-2 h-2 rounded-full ${status === 'on_lunch_break' || screenCapturePaused
                ? 'bg-yellow-500'
                : screenCaptureActive
                  ? 'bg-green-500 animate-pulse'
                  : 'bg-red-500'
        ...
        <div className="grid grid-cols-2 gap-4 mt-auto">
          ...
          {(status === 'punched_out' || status === 'on_leave') && (
            <div className={`col-span-2 ${status === 'on_leave'
